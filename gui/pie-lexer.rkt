...
(define (pie-lexer in)
  (let-values ([(lexeme type paren start end adj) (pie-lexer/status in)])
...
(define-syntax-rule (lexer/status identifier keyword bad-id)
  (lexer
   ...
   [(:: "'" identifier)
    (ret lexeme 'data-ctor #f start-pos end-pos 'continue)]
   [(:or "'" "`" "#'" "#`" "#&")
    ...
    (let ((category
           (case lexeme
             [("claim"
               "check-same"
               "TODO"
               "define" "which-Nat" "iter-Nat" "rec-Nat" "ind-Nat"
               "ind-Either" "head" "tail" "ind-Vec" "ind-Absurd" "the"
...
(define pie-lexer/status (lexer/status identifier keyword bad-id))
(define pie-nobar-lexer/status (lexer/status nobar-identifier nobar-keyword nobar-bad-id))
